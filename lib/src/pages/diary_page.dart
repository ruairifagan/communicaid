...
class DiaryCategoryPage extends StatefulWidget {
  static const String routeName = '/diary-category-page';
  ...
  @override
  _DiaryCategoryPageState createState() => _DiaryCategoryPageState();
}

class _DiaryCategoryPageState extends State<DiaryCategoryPage> {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (ctx, index) {
                          return DiaryEntryItem(
                            entryDocument: snapshot.data[index],
                          );
                        },
                      );
                    },
  ...
  Future<Stream<List<DocumentSnapshot>>> getListOfEntries() async {
    return _firestore.collection('diary').document(currentUserId).collection(currentUserId).document(widget.docId).collection(widget.docId).snapshots().transform(
      StreamTransformer<QuerySnapshot, List<DocumentSnapshot>>.fromHandlers(
        handleData: (querySnapshot, sink) async {
          ...
          final _entryList = List<DocumentSnapshot>();
